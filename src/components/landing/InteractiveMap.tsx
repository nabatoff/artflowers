      ...
      <AnimatePresence>
        {isHovered && (
          <motion.div
            ...
            transition={{ duration: 0.2 }}
            className="absolute bottom-full left-1/2 -translate-x-1/2 mb-2 px-3 py-1.5 rounded-lg bg-background border border-border shadow-lg whitespace-nowrap z-20"
            style={{ pointerEvents: 'none' }}
          >
            <p className="text-sm font-medium text-foreground">{country.name}</p>
            {/* Tooltip arrow */}
            <div className="absolute top-full left-1/2 -translate-x-1/2 -mt-px">
              <div className="w-2 h-2 bg-background border-r border-b border-border transform rotate-45" />
            </div>
          </motion.div>
        )}
      </AnimatePresence>
